...
discharge_folder='cache/BAM_Discharge/6/'
...
for(reachID in idlist){
  ...
  # file_date=paste(discharge_folder,reachID,'_', widthID,'_','dates.rds',sep="")
  ...
  colnames(output)=c('Date','time', 'Gauge_Q',paste0('Bam_Q_',c(20:20)))
  ...
  local({
    ...
    nm = reachID
    ymin = min(mydata[,3:4],na.rm=T)
    ymax = max(mydata[,3:4],na.rm=T)
    pl <- ggplot() + 
  ...
  metrics_out = metrics(output$Gauge_Q,output[,4])[3]
  #print(metrics_out)
  out = metrics_out
