...
#     height = 15.24, width = 17.78, units = 'cm', res = 1200)
...
ts1_1=unique(ts1_1)
par(fig=c(0,4,6.8,10)/10,new=TRUE)
...
par(fig=c(4,10,6.8,10)/10,new=TRUE)
...
ts3_1=unique(ts3_1)
par(fig=c(0,10,3.6,6.8)/10,new=TRUE)  #x1,x2,y1,y2
...
colnames(output_all)=c('Date','Gauge_Q',width_list[3])
output_all = output_all[order(output_all$Date),]
colnames(output_all1)=c('Date','Gauge_Q',width_list[3])
output_all1 = output_all1[order(output_all1$Date),]
#tiff(paste0("cache/discharge/01/",reachID,"_combined.tiff"), 
#height = 5.842, width = 17.78, units = 'cm', res = 1200)
ymin=min(output_all[,2:3],na.rm=T)*0.3
ymax=max(output_all[,2:3],na.rm=T)*0.8
#size = 1.26
#windows(7,2.3)
#par(mar=c(2.5,3.0,0,1.0),mgp=c(1.5,0.5,0),family = "serif")
#layout(matrix(c(1,2,3,3), 2, 2, byrow = TRUE))
ts1 = output_all[complete.cases(output_all[,3]),1:3]
ts1_1 = output_all1[complete.cases(output_all1[,3]),1:3]
par(fig=c(0,10,0,3.6)/10,new=TRUE)  #x1,x2,y1,y2
par(mar=c(3.2,3.5,0.7,1.1),mgp=c(1.6,0.5,0),family = "serif")
plot(c(1:nrow(ts1)),ts1$Gauge_Q,font=1,type='l',col='black',tck=0.02,lwd=2,xlab='Time Step',cex.lab=size,cex.axis=size,
     ylab='',
     ylim=c(ymin,ymax))
lines(c(1:nrow(ts1)),ts1$PL,col='red')
lines(c(1:nrow(ts1_1)),ts1_1$PL,col='blue')
text(x=0.6,ymax*0.98,labels='d',font=2,cex=1.5*size)
