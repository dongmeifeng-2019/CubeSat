...
for(reachID in idlist){
  ...
  for(width_input in width_list[4:5]){
    ...
    out = data.frame(Date=bamdates,rc_Q=apply(Q_inv,2, function(x) median(x,na.rm=TRUE)),gauge_Q=exp(logq_matrix[1,]))
    write.table(out,paste(discharge_folder,reachID,'_',width_input,'.txt',sep=''),col.names = T,row.names = F)
    #write.table(t(AHG),'D:/RiverWidth/paper1/figure8.txt',col.names = F,row.names = F,append = T)
    #write.table(pval,'D:/RiverWidth/paper1/figure8.txt',col.names = F,row.names = F,append = T)
