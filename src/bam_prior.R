...
if(priorID==1){
  ...
  if(priorID==2){
    prior_folder='data/Priors/NCEP/'
  }
  if(priorID==3){
    prior_folder='data/Priors/MERRA-2/'
  }
  if(priorID==4){
    prior_folder='data/Priors/ERA5/'
  }
  prior_in=read_delim(paste(prior_folder,reachID,'.txt',sep=""),delim=" ") %>%
    transmute(prior_Q= as.integer(q_cfs)*(.3048^3), Date= time  ) 
  prior_in[prior_in==0]=NA
  prior_in = data.frame(Date=t)%>%
    left_join(prior_in,by='Date')
  logQc_hat = log(mean(prior_in$prior_Q,na.rm=TRUE)/(sqrt(1+ var(prior_in$prior_Q,na.rm=TRUE)/(mean(prior_in$prior_Q,na.rm=TRUE)^2))))
  logQc_sd = log(1  + var(prior_in$prior_Q,na.rm=TRUE)/ (mean(prior_in$prior_Q,na.rm=TRUE))^2  )
  lowerbound_logQ =  log(min(prior_in$prior_Q,na.rm=TRUE))  #logQc_hat-0.1 #
  upperbound_logQ = log(max(prior_in$prior_Q,na.rm=TRUE))  #logQc_hat #
}
