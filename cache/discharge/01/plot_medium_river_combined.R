...
ymax=max(output_all[,2],na.rm=T)*0.5
size = 1.5
...
plot(c(1:nrow(ts1)),ts1$Gauge_Q,font=2,type='l',col='black',tck=-0.03,lwd=2,cex.lab=size,cex.axis=size,
...
text(x=1,ymax,labels='a',font=2,cex=1.5*size)
...
text(x=1,ymax,labels='b',font=2,cex=1.5*size)
...
plot(c(1:nrow(ts3)),ts3$Gauge_Q,font=2,type='l',col='black',tck=-0.03,lwd=2,xlab='Time Step',cex.lab=size,cex.axis=size,
     ylab=expression(paste('Discharge (m'^3,'/s)',sep='')),
     ylim=c(ymin,ymax))
lines(c(1:nrow(ts3)),ts3$'ST-PL',col='red')
lines(c(1:nrow(ts3_1)),ts3_1$'ST-PL',col='blue')
text(x=0.5,ymax,labels='c',font=2,cex=1.5*size)
#dev.off()
